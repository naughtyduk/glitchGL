...
const glitchGL = (function () {
  ...
  class GlitchGLRenderer {
    ...
    async loadModel(modelSrc, scene, options) {
      return new Promise((resolve, reject) => {
        if (!GLTFLoader) {
          console.error("glitchGL: GLTFLoader is required for model loading.");
          return reject("Missing GLTFLoader.");
        }
   ...
   *  Public API
   * ------------------------------------------------*/
  const glitchGL = function (userOptions = {}) {
    ...
    const options = glitchGL.mergeDeep(defaults, userOptions);
  ...
  glitchGL.mergeDeep = function (target, source) {
    const output = Object.assign({}, target);
    if (glitchGL.isObject(target) && glitchGL.isObject(source)) {
      Object.keys(source).forEach((key) => {
        if (glitchGL.isObject(source[key])) {
          if (!(key in target)) Object.assign(output, { [key]: source[key] });
          else output[key] = glitchGL.mergeDeep(target[key], source[key]);
        } else {
          Object.assign(output, { [key]: source[key] });
        }
      });
  ...
  glitchGL.isObject = function (item) {
    return !!(item && typeof item === "object" && !Array.isArray(item));
  };
